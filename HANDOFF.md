...
再計数担当が利用上限で停止したため、全量計数の代わりに**標本ベースの比率**を出した。`recount_ratio.py` と `artifacts-recount-ratio.json`。既知の32k計数値を `ratio` で再スケールすれば予算判断ができる。担当者復帰後の全量計数の検算にも使える（決定的な標本・同一seed なので再現する）。

| ソース | 旧 c/t | 新 c/t | 削減 | ドメイン |
|---|---|---|---|---|
| kasys-ReCaRe | 1.584 | 1.806 | **12.31%** | ja_reference_formal |
| finepdfs | 1.571 | 1.753 | **10.37%** | ja_long_form |
| wikisource | 1.477 | 1.607 | 8.10% | ja_reference_formal |
| wikipedia ja | 1.609 | 1.728 | 6.85% | ja_reference_formal |
| llm-jp-scaling (ja_web) | 2.004 | 2.136 | 6.16% | ja_high_quality_web |
| llm-jp-continuation | 2.009 | 2.108 | 4.71% | paraphrase / doc-QA |
| dclm-edu | 3.571 | 3.703 | 3.57% | en_education_science |
| finemath | 2.902 | 2.995 | 3.09% | math_reasoning |
| PleIAs-Japanese-PD | 1.144 | 1.173 | 2.48% | ja_long_form |
| stack-edu pilot | 2.881 | 2.895 | 0.48% | code |
| **stack-edu r3** | 2.392 | 2.387 | **−0.23%** | code |

**削減率は分野ごとに大きく違う。集約値の 5.43% で判断すると外す。**

**`stack-edu-continuation-r3` だけ悪化している** — コードは 48k でわずかに増える。日本語へ語彙を振った分コードが押し出された結果で、報告の「コード 0.94% 改善」とは逆符号（pilot と r3 で違うので、担当者が測ったのは pilot の可能性がある）。害は小さいが code の候補量は縮まないので予算計算では別扱いになる。

### ja_reference_formal は目標割れする

この領域は全ソースが 6.85〜12.31% で、**全部が平均を上回る**。

```
Wikipedia     1,610,075,462 × 0.9315 = 1,499,905,000
Wikisource       44,450,893 × 0.9190 =    40,850,000
ReCaRe           20,227,144 × 0.8769 =    17,737,000
finepdfs許諾    102,964,461 × 0.8963 =    92,287,000
候補計                                 1,650,779,000
目標                                   1,600,000,000
余裕                                      50,779,000 = 3.2%   ← 重複除去の前
```

候補の90.6%が Wikipedia で、Wikipedia 由来は finepdfs や web crawl に大量にミラーされている。**横断重複除去で削られる本命がここ。3.2%では耐えられない可能性が高い。**

対して `ja_long_form` は候補 3,708,034,081 → 約 3,326,000,000、目標 2,400,000,000 で**余裕 38.6%（約9.3億トークンの余剰）**。

**振り替えが素直**（例: 0.10 → 0.09 / 0.15 → 0.16 で formal 余裕 15%・long_form 余裕 28.8%）。合計16Bは不変で、訓練開始前に決めるので「固定比率フェーズ中に重みを変えない」原則も破らない。許諾済み公文書（e-Gov / MEXT / MHLW / MAFF、利用規約確認済み）を積む手もあるが取得作業が増える。
